...
CREATE TABLE messages_step5 (
    id TEXT PRIMARY KEY CHECK(length(id) = 36), meeting_id TEXT NOT NULL, event_id TEXT NOT NULL UNIQUE,
    author_kind TEXT NOT NULL CHECK(author_kind IN ('host', 'guest')), member_id TEXT NULL,
    guest_session_id TEXT NULL, display_name_snapshot TEXT NOT NULL CHECK(trim(display_name_snapshot) <> ''),
    content TEXT NOT NULL CHECK(trim(content) <> ''), created_at INTEGER NOT NULL CHECK(created_at >= 0),
...
INSERT INTO messages_step5 (
    id, meeting_id, event_id, author_kind, member_id, guest_session_id,
    display_name_snapshot, content, created_at, updated_at
)
SELECT id, meeting_id, event_id, author_kind, member_id, guest_session_id,
       display_name_snapshot, content, created_at, updated_at
FROM messages;
DROP TABLE messages;
ALTER TABLE messages_step5 RENAME TO messages;
...
CREATE TABLE resources_step5_rollback (
    id TEXT PRIMARY KEY CHECK(length(id) = 36), meeting_id TEXT NOT NULL, event_id TEXT NOT NULL UNIQUE,
...
INSERT INTO resources_step5_rollback (
    id, meeting_id, event_id, guest_session_id, kind, original_name, safe_name, relative_path,
    source_url, media_type, size_bytes, sha256, original_description, current_description,
    description_revision, state, created_at, updated_at
)
...
ALTER TABLE resources_step5_rollback RENAME TO resources;
