-- 本回退仅用于未发布开发库；显式重建 resources 以移除完整性校验字段。
PRAGMA foreign_keys = OFF;

DROP INDEX IF EXISTS idx_meetings_started_no;
DROP INDEX IF EXISTS idx_meeting_participants_snapshot_meeting;

CREATE TABLE resources_step8 (
...
SELECT id, meeting_id, event_id, guest_session_id, request_id, kind, original_name, safe_name,
       relative_path, source_url, media_type, size_bytes, sha256, original_description,
       current_description, description_revision, state, created_at, updated_at
FROM resources;
DROP TABLE resources;
ALTER TABLE resources_step8 RENAME TO resources;
